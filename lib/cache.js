...
const runTokenBurnInWorker = ({
  ...
  const worker = new WorkerImpl(path.join(__dirname, 'token-burn-worker.js'), {
    workerData: { pricingSnapshot: getOpenRouterPricingSnapshotImpl() },
...
const runGitBlameInWorker = (WorkerImpl = Worker) => new Promise((resolve, reject) => {
...
function startBackgroundUpdater({
  ...
  const updateTokens = async () => {
    ...
    try {
      cache.tokensData = await runTokenBurnInWorker({ WorkerImpl, getOpenRouterPricingSnapshotImpl });
    } catch (err) {
  ...
  const warmGitBlame = async () => {
    ...
    try {
      const data = await runGitBlameInWorker(WorkerImpl);
...
module.exports = {
  ...
  getHistoricalData,
  __resetForTesting,
  __GIT_BLAME_WARM_CWD_FOR_TESTING: GIT_BLAME_WARM_CWD
};
