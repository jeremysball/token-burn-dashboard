    ...
    for (const line of lines) {
      try {
        ...
        if (data.type === 'message' && data.message?.usage) {
          ...
          const total = usage.totalTokens || (input + output + cacheRead + cacheWrite + Number(reasoning));

          result.total_input += input;
          result.total_output += output;
          result.total_cache_read += cacheRead;
          result.total_cache_write += cacheWrite;
          result.total_reasoning += Number(reasoning) || 0;
          ...
          result.models[modelKey].reasoning += Number(reasoning) || 0;
    ...
    try {
        ...
        for (const sessionDir of sessionDirs) {
          ...
          for (const filePath of jsonlFiles) {
            ...
            result.total_reasoning += fileData.total_reasoning;
