...
function runTokenBurn() {
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      /** @type {{ files_processed: number, total_lines: number, total_messages: number, total_input: number, total_output: number, total_cache_read: number, total_cache_write: number, total_reasoning: number, total_tokens: number, tokens_by_model: Record<string, {input: number, output: number, cache_read: number, cache_write: number, reasoning: number, total: number}>, costs_by_model: Record<string, {input: number, output: number, cache_read: number, cache_write: number, reasoning: number, total: number}>, pricing_by_model: Record<string, *>, total_cost: {input: number, output: number, cache_read: number, cache_write: number, reasoning: number, total: number} }} */
      const result = {
        files_processed: 0,
        total_lines: 0,
        total_messages: 0,
      ...
      clearTimeout(timeoutId);
      reject(new Error(`Token burn failed: ${err instanceof Error ? err.message : String(err)}`));
    }
  });
}
