...
async function handleGitBlameRoute(req, res, requestTimeout) {
  try {
    const { generateGitBlameReport, getCostByFile, getAvailableDirectories, getCommitSessionDetails } = require('../git-blame');
    ...
    if (commitHash) {
      const details = getCommitSessionDetails(commitHash, cwd, days);
      clearTimeout(requestTimeout);
      if (!details) {
    ...
    const report = generateGitBlameReport(days, cwd);
    const fileCosts = getCostByFile(days, cwd);
    const directories = getAvailableDirectories();
    
    clearTimeout(requestTimeout);
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ commits: report, files: fileCosts, directories }));
  } catch (err) {
    clearTimeout(requestTimeout);
    res.writeHead(500, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ error: err.message }));
