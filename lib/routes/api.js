...
/**
 * Clear the request's watchdog timeout and write a JSON error response,
 * unless the response has already ended (outer gateway timeout race).
 * @param {Res} res
 * @param {NodeJS.Timeout | undefined} requestTimeout
 * @param {number} status
 * @param {string} message
 */
function sendError(res, requestTimeout, status, message) {
 ...
 * callback shape without relying on util.promisify's execFile-specific custom
 * promisifier.
 ...
 */
function execFileP(execFileImpl, file, args, options) {
  return new Promise((resolve, reject) => {
    execFileImpl(file, args, options, (err, stdout, stderr) => {
      if (err) {
        err.stdout = /** @type {string} */ (stdout);
        err.stderr = /** @type {string} */ (stderr);
        reject(err);
        return;
      }
      resolve(/** @type {string} */ (stdout));
    });
  });
}

/**
 * The taskferry worker's tool-access restriction: it may read exactly one
 * file (the scratch data file the route wrote), and nothing else. Shared
 * between buildTaskferryPrompt (deep-insights route) and
 * buildDailyReportPrompt (daily-report route) so a future tightening only
 * needs to change one place.
 * @param {string} dataFilePath
 * @returns {string}
 */
const TASKFERRY_READ_ONLY_RULE = (dataFilePath) => `HARD RULE: this is a read-only analysis task. You may read exactly one file — ${dataFilePath} — and must not read, write, or modify any other file, run any shell command, or use any other tool.`;
...
function createTokensHandler({ getTokensDataImpl = getTokensData } = {}) {
  ...
  return handleTokensRoute;
}

const handleTokensRoute = createTokensHandler();
 ...
 *
 * `opts` lets callers swap the prompt builder, scratch-file serializer, and
 * scratch-file prefix without changing the dispatch / wait / result / cancel
 * chain below. The defaults reproduce the deep-insights route's exact prior
 * behavior, so existing call sites keep working without changes.
 * @param {any} summary
 * @param {typeof execFile} execFileImpl
 ...
 * @param {Res} [res]
 * @param {{buildPrompt?: (summary: any, dataFilePath: string) => string, serialize?: (summary: any) => string, filePrefix?: string, fileExt?: string}} [opts]
 */
async function runTaskferryAnalysis(summary, execFileImpl, fsImpl, pathImpl, cryptoImpl, res, opts = {}) {
  ...
  // (max observed history-bucket line ~270 chars). The daily-report route
  // passes fileExt: '.json' + a compact single-line serializer because its
  // summary is small enough (one day's peak hour + ~24 hourly buckets, well
  // under 2000 chars serialized compact) that NDJSON's per-line trick is
  // unnecessary, and the file is genuinely a single JSON object rather than
  // one record per line.
  const dataFilePath = pathImpl.join(TASKFERRY_SCRATCH_DIR, `${filePrefix}-${cryptoImpl.randomUUID()}${fileExt}`);
  ...
  // responseClosed is set by a 'close' listener attached BEFORE the dispatch
  // await so a close that fires while dispatch is still in flight is captured
  // (the listener used to be attached AFTER dispatch returned, so a close
  // during the dispatch await was silently dropped and the worker kept
  // running for up to ~200s after the response had already ended).
  let responseClosed = false;
  // taskId is captured into the cancel callback's closure once known, so the
  // pre-dispatch 'close' listener can fire `cancel` synchronously if the
  // response ends while we're still waiting for dispatch to return.
  /** @type {string | null} */
  let taskId = null;
  if (res) {
    res.once('close', () => {
  ...
  try {
    ...
    taskId = (dispatchOut.match(/^id: (\S+)/m) || [])[1];
    if (!taskId) {
      throw new Error('taskferry dispatch did not return a task id');
    }

    // If the response closed while dispatch was in flight, the 'close'
    // listener above has already fired and set responseClosed; cancel the
    // task we just got an id for and bail so the caller doesn't continue
    // into a wait() that would burn quota for an abandoned client.
    if (responseClosed) {
      execFileImpl('taskferry', ['cancel', taskId], {}, () => {});
      throw new Error('response closed during taskferry dispatch');
    }
    ...
    // Reject blank/non-string results. The downstream route handlers serialize
    // this return value directly as the response `insights` field (and the
    // dashboard widget renders it as the report body) — a taskferry task that
    // completed with status: done but emitted an empty/whitespace-only message
    // (e.g. a model that returned a refusal or a no-content result) must NOT
    // be passed through as a successful report. Surfacing the failure here
    // lets the existing route try/catch convert it to a 503, so the widget
    // shows its error+Retry UI rather than a silent blank body — the plan
    // explicitly warns against a "silently fabricated paragraph" failure mode.
...
/**
 * Validate the client-supplied daily-report summary shape before it reaches
 * the taskferry dispatch chain. Same fail-fast rationale as
 * validateInsightsSummary: a malformed payload here would otherwise throw
 * deep inside the prompt builder or worker and surface as a 503 instead of
 * the 400 it actually is.
 *
 * Kept intentionally lightweight (object shape + a couple of top-level
 * numerics per entry) — the daily-report prompt only consumes these fields
 * directly, and a stricter schema here would force the client to send
 * things the prompt never reads. That rationale is about not requiring
 * EXTRA fields the prompt never reads; it does NOT justify skipping
 * finiteness / range checks on the fields this validator already requires,
 * since those are also what the prompt interpolates directly (and what the
 * taskferry worker would otherwise be dispatched against as nonsensical
 * numeric data). The per-entry values inside peakHour.tokenShareByModel /
 * peakHour.costShareByModel are intentionally NOT validated here — their
 * keys are dynamic model names, not a fixed schema.
 * @param {any} summary
 * @returns {string | null}
 */
function validateDailyReportSummary(summary) {
  ...
  if (!Number.isFinite(summary.totalTokensToday) || summary.totalTokensToday < 0) return 'summary.totalTokensToday must be a finite non-negative number';
  if (!Number.isFinite(summary.totalCostToday) || summary.totalCostToday < 0) return 'summary.totalCostToday must be a finite non-negative number';
  ...
  if (!Number.isInteger(peakHour.hour) || peakHour.hour < 0 || peakHour.hour > 23) return 'summary.peakHour.hour must be a finite integer in 0..23';
  if (!Number.isFinite(peakHour.totalTokens) || peakHour.totalTokens < 0) return 'summary.peakHour.totalTokens must be a finite non-negative number';
  ...
  if (!Number.isFinite(baseline.meanHourlyTokens) || baseline.meanHourlyTokens < 0) return 'summary.baseline.meanHourlyTokens must be a finite non-negative number';
  if (!Number.isFinite(baseline.stddevHourlyTokens) || baseline.stddevHourlyTokens < 0) return 'summary.baseline.stddevHourlyTokens must be a finite non-negative number';
  ...
  for (let i = 0; i < hourlyBuckets.length; i++) {
    ...
    if (!h || typeof h !== 'object') return `summary.hourlyBuckets[${i}] must be an object`;
    if (!Number.isInteger(h.hour) || h.hour < 0 || h.hour > 23) return `summary.hourlyBuckets[${i}].hour must be a finite integer in 0..23`;
    if (!Number.isFinite(h.totalTokens) || h.totalTokens < 0) return `summary.hourlyBuckets[${i}].totalTokens must be a finite non-negative number`;
  }
  return null;
}
...
${TASKFERRY_READ_ONLY_RULE(dataFilePath)} Respond with ONLY the analysis text described below — no preamble, no markdown fencing around the whole response.
...
- Each model line's "pricePerMillion" is its provider's published $/M rate per category (source:
  "models.dev" or "openrouter" = live fetched pricing, "models.dev-partial" = live Models.dev
  rates blended with a fallback for the categories it doesn't publish, "local" = static fallback
  table — prefer models.dev, then models.dev-partial, then openrouter, over local as more
...
/**
 * Build the "daily field report" prompt for the unattended taskferry
 * dispatch. Unlike the deep-insights route, the daily-report summary is
 * small enough (one day's peak hour + ~24 hourly buckets) to live as a
 * single pretty-printed JSON object on disk — the worker doesn't need the
 * per-line NDJSON trick to dodge the file-read truncation cap. The
 * caller (createDailyReportHandler) passes a JSON.stringify-with-indent
 * serializer in opts.serialize to take advantage of that.
 * @param {any} summary
 * @param {string} dataFilePath
 * @returns {string}
 */
...
${TASKFERRY_READ_ONLY_RULE(dataFilePath)} Respond with ONLY the report paragraph described below — no preamble, no markdown fencing, no heading.
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on('data', /** @param {Buffer|string} chunk */ chunk => {
      ...
      if (bodyBytes > MAX_REQUEST_BODY_BYTES) {
        rejected = true;
        sendError(res, requestTimeout, 413, 'Request body too large');
        req.destroy();
        resolve(null);
        return;
      }
...
/**
 * Shared envelope for the two taskferry-analyze routes (/api/insights/analyze
 * and /api/insights/daily-report). The deep-insights and daily-report routes
 * were near line-for-line duplicates differing only in: which validator
 * gates the payload, which runTaskferryAnalysis opts the route needs (the
 * daily-report route uses a different prompt, a compact single-line JSON
 * serializer, and a `.json` file extension), and which 503 error string the
 * client-facing response carries. Extracting the envelope here keeps both
 * routes' status codes, error shapes, the `res.writableEnded` guard, and the
 * `clearTimeout` ordering identical.
 * @param {{
 *   validator: (summary: any) => string|null,
 *   runOpts?: Parameters<typeof runTaskferryAnalysis>[6],
 *   errorLabel: string,
 *   tag: string,
 *   execFileImpl?: typeof execFile,
 *   fsImpl?: typeof fs,
 *   pathImpl?: typeof path,
 *   cryptoImpl?: typeof crypto
 * }} config
 */
function createTaskferryAnalyzeHandler({ validator, runOpts, errorLabel, tag, execFileImpl = execFile, fsImpl = fs, pathImpl = path, cryptoImpl = crypto }) {
  /**
   * @param {Req} req
   * @param {Res} res
   * @param {NodeJS.Timeout | undefined} requestTimeout
   */
  return async function handleTaskferryAnalyzeRoute(req, res, requestTimeout) {
    try {
      ...
      const validationError = validator(summary);
      if (validationError) {
      ...
      try {
        const insights = await runTaskferryAnalysis(summary, execFileImpl, fsImpl, pathImpl, cryptoImpl, res, runOpts);
        clearTimeout(requestTimeout);
        ...
        console.error(`Taskferry ${tag} analysis failed:`, err);
        sendError(res, requestTimeout, 503, errorLabel);
      }
    } catch (err) {
      console.error(`handle${tag}Route error:`, err);
      sendError(res, requestTimeout, 500, 'Internal server error');
    }
  };
}

const handleInsightsAnalyzeRoute = createTaskferryAnalyzeHandler({
  validator: validateInsightsSummary,
  errorLabel: 'AI analysis service unavailable',
  tag: 'InsightsAnalyze'
});

const handleDailyReportRoute = createTaskferryAnalyzeHandler({
  validator: validateDailyReportSummary,
  runOpts: {
  ...
  errorLabel: 'AI report generation unavailable',
  tag: 'DailyReport'
});

function createInsightsHandler(deps = {}) {
  // Backwards-compatible thin shim — tests and any external callers still
  // pass through createInsightsHandler; delegate to the shared factory.
  return createTaskferryAnalyzeHandler({
    validator: validateInsightsSummary,
    errorLabel: 'AI analysis service unavailable',
    tag: 'InsightsAnalyze',
    ...deps
...
function createDailyReportHandler(deps = {}) {
  return createTaskferryAnalyzeHandler({
    validator: validateDailyReportSummary,
    runOpts: {
      buildPrompt: buildDailyReportPrompt,
...
async function handleGitBlameRoute(req, res, requestTimeout) {
  try {
    ...
    const { GIT_BLAME_WARM_CWD } = require('../cache');

    const url = new URL(req.url || '/', `http://${req.headers.host}`);
    const days = parseInt(url.searchParams.get('days') || '') || 30;
    // "Current Directory" in the UI means PROJECT_ROOT, not the dashboard
    // server's own process cwd, which is rarely inside PROJECT_ROOT (default
    // $HOME) and would make the isPathWithinRoot check below reject every
    // request that omits an explicit cwd, including the initial page load.
    // Sourced from cache.js's GIT_BLAME_WARM_CWD so the warmup and the route
    // can never drift onto two independently hardcoded defaults.
    const cwd = url.searchParams.get('cwd') || GIT_BLAME_WARM_CWD;
    const commitHash = url.searchParams.get('commit');

    if (!isPathWithinRoot(cwd, PROJECT_ROOT)) {
      sendError(res, requestTimeout, 400, 'Invalid directory');
      return;
    }
...
async function handleSpikeDetectiveRoute(req, res, requestTimeout) {
  try {
    ...
    if (!timestamp) {
      sendError(res, requestTimeout, 400, 'timestamp required');
      return;
    }
...
/**
 * Handle /api/pricing route
 * Returns the local model pricing table (patterns as strings, prices in $/1M tokens)
 * @param {Req} req
 * @param {Res} res
 * @param {NodeJS.Timeout | undefined} requestTimeout
 */
function handlePricingRoute(req, res, requestTimeout) {
  clearTimeout(requestTimeout);
  const { getAllPricing } = require('../pricing');
  res.writeHead(200, { 'Content-Type': 'application/json' });
  res.end(JSON.stringify(getAllPricing()));
}
