...
function buildAnalysisPrompt(summary) {
  const topModels = summary.topModels.map(m => {
    const effectiveRate = m.tokens ? (m.cost / (m.tokens / 1e6)) : 0;
    const rates = m.pricePerMillion
      ? `list price $${m.pricePerMillion.input}/M in, $${m.pricePerMillion.output}/M out, $${m.pricePerMillion.cacheRead}/M cache-read`
      : 'list price unknown';
    return `- ${m.name}: ${(m.tokens / 1e6).toFixed(2)}M tokens (${(m.inputTokens / 1e6).toFixed(2)}M in / ${(m.outputTokens / 1e6).toFixed(2)}M out / ${(m.cacheReadTokens / 1e6).toFixed(2)}M cache-read), $${m.cost.toFixed(2)} actual spend ($${effectiveRate.toFixed(2)}/M effective), ${(m.cacheRate * 100).toFixed(0)}% cache hit rate, ${rates}`;
  }).join('\n');

  return `Analyze this LLM usage data and provide 3-4 specific, actionable insights. Use ONLY the numbers given below — do not invent model names, rates, or dollar figures that are not present in this data.

**Overview:**
- Total tokens: ${(summary.totalTokens / 1e9).toFixed(2)}B
- Total cost: $${summary.totalCost.toFixed(2)}
- Distinct models used: ${summary.modelCount} (top ${summary.topModels.length} shown below, by token volume)
...
Each model's "list price" is its provider's published $/M token rate; "effective" is what was
actually paid per million tokens across its own input/output/cache mix in this data — these can
differ. Only compare models that both actually appear in the Top Models list above.

Focus on:
1. Cost optimization opportunities (cite specific dollar savings computed from the numbers above)
2. Model selection strategy (which of the listed models to use more/less, and why)
3. Cache utilization improvements (which listed models have low cache hit rates)
4. Workload pattern observations (from the input/output ratio and cache rates above)
