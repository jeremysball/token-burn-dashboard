...
function validateDailyReportSummary(summary) {
  ...
  if (!Number.isFinite(peakHour.hour) || peakHour.hour < 0 || peakHour.hour > 23) return 'summary.peakHour.hour must be a finite integer in 0..23';
  ...
  for (let i = 0; i < hourlyBuckets.length; i++) {
    ...
    if (!Number.isFinite(h.hour) || h.hour < 0 || h.hour > 23) return `summary.hourlyBuckets[${i}].hour must be a finite integer in 0..23`;
...
function createInsightsHandler({ execFileImpl = execFile, fsImpl = fs, pathImpl = path, cryptoImpl = crypto } = {}) {
  /**
  ...
  async function handleInsightsAnalyzeRoute(req, res, requestTimeout) {
    try {
      ...
      try {
        ...
        sendError(res, requestTimeout, 503, 'AI analysis service unavailable');
      }
    } catch (err) {
  ...
  return handleInsightsAnalyzeRoute;
}

const handleInsightsAnalyzeRoute = createInsightsHandler();
