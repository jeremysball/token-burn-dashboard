...
function handleStaticRoutes(url, res, requestTimeout, rootDir) {
  // In production, serve the Vite-built bundle (dist-dashboard/) instead of
  // the raw dashboard/ source; in development, serve dashboard/ directly.
  const isProd = process.env.NODE_ENV === 'production';
  const dashboardSourceDir = isProd ? 'dist-dashboard' : 'dashboard';

  // Static root mappings
  /** @type {Record<string, string>} */
  const staticMap = {
    '/': path.join(rootDir, dashboardSourceDir, 'index.html')
  };
  ...
  if (url.pathname.startsWith('/dashboard/')) {
    ...
    const dashboardRoot = path.join(rootDir, dashboardSourceDir);
  ...
  // Root-relative asset requests (Vite's built /assets/*.js|css, and dev-mode
  // /styles/*, /js/* referenced by index.html) map directly into
  // dashboardSourceDir. Only handle requests that look like a file (have an
  // extension), so route-like paths without extensions still fall through to
  // the 404 handler below instead of being swallowed here.
  if (path.extname(url.pathname)) {
    const assetPath = safeStaticPath(path.join(rootDir, dashboardSourceDir), url.pathname);
    if (!assetPath) {
      clearTimeout(requestTimeout);
      res.writeHead(403, { 'Content-Type': 'text/plain' });
      res.end('Forbidden');
    ...
    const ext = /** @type {keyof typeof MIME_TYPES} */ (path.extname(assetPath));
    clearTimeout(requestTimeout);
    serveStatic(res, assetPath, MIME_TYPES[ext] || 'text/plain');
    return true;
  }
