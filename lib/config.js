...
function loadConfig(env = process.env, cwd = process.cwd()) {
  return {
    ...
    TASKFERRY_INSIGHTS_MODEL: env.TASKFERRY_INSIGHTS_MODEL || 'opencode/deepseek-v4-flash-free',
    TASKFERRY_SCRATCH_DIR: env.DASHBOARD_INSIGHTS_SCRATCH_DIR
      || path.join(env.XDG_CACHE_HOME || path.join(os.homedir(), '.cache'), 'token-burn-dashboard', 'insights-scratch'),

    // Security hardening
    MAX_REQUEST_BODY_BYTES: 1024 * 1024,
    MAX_FILE_BYTES: parseInt(env.MAX_SESSION_BYTES || '', 10) || 100 * 1024 * 1024,
    CLAUDE_MAX_DEPTH: 4,
