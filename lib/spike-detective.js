...
const { PI_SESSION_BASES: SESSIONS_PATHS } = require('./session-paths');
    ...
    for (const line of lines) {
      try {
        ...
        if (data.type === 'message' && data.message) {
          ...
          messageCount++;
          totalTokens += usage.totalTokens ?? ((usage.input || usage.inputTokens || 0) + (usage.output || usage.outputTokens || 0) + (usage.cacheRead || 0) + (usage.cacheWrite || 0));
          ...
          // Calculate cost using shared pricing
          const costTokens = {
          ...
          /** @type {*} */
          const costs = calculateCost(costTokens, msg.model || '');
