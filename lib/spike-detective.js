...
function getSessionsInWindow(startTime, endTime) {
  ...
  for (const file of findAllSessionFiles()) {
    ...
    const fileData = getCachedFileData(file.path, { collectEvents: false, collectPreviews: true });
    const timeWindow = fileData.timeWindow;
    if (!timeWindow || timeWindow.startTime > endTime || timeWindow.endTime < startTime) continue;

    const conversation = summarizeSessionData(fileData);
 ...
 * Takes the parsed fileData (from getCachedFileData with collectPreviews:
 * true) rather than a filePath+re-parsing, since the caller already had to
 * parse the file once to check time-window overlap, and previews are
 * collected inline during that same parse pass.
 * @param {*} fileData - result of getCachedFileData(filePath, {collectPreviews: true})
 ...
 */
function summarizeSessionData(fileData) {
  let totalCost = 0;
