...
function queryJsonSafe(sql, timeoutMs = 15000) {
  ...
  try {
    ...
    console.error(`Opencode DB query failed: ${/** @type {Error} */ (err).message}`);
    return [];
  }
}
...
function getOpenCodeSessions() {
  ...
  const sessions = rows.map(/** @param {Record<string, unknown>} row */ (row) => {
    let modelId = 'unknown';
    ...
    return {
      ...
      total: (Number(row.input) || 0) + (Number(row.output) || 0) + (Number(row.cache_read) || 0) + (Number(row.cache_write) || 0) + (Number(row.reasoning) || 0),
...
function getOpenCodeHistoricalEvents(limit = 10000) {
  ...
  return rows.map(/** @param {Record<string, unknown>} row */ (row) => {
    ...
    return {
      ...
      cache_write: Number(row.cache_write) || 0,
      reasoning: Number(row.reasoning) || 0,
      total: row.total != null ? Number(row.total) : ((Number(row.input)||0)+(Number(row.output)||0)+(Number(row.cache_read)||0)+(Number(row.reasoning)||0)),
...
function getOpenCodeTotalsByModel() {
  ...
  /** @type {Record<string, {input: number, output: number, cache_read: number, cache_write: number, reasoning: number, total: number, cost: number, sessions: number, source: string, provider: string, model: string}>} */
  const byModel = {};
