...
/**
 * More robust query using temporary file for SQL to avoid shell escaping issues
 */
function queryJsonSafe(sql, timeoutMs = 15000) {
  if (!dbExists()) return [];

  const tmpFile = `/tmp/opencode_query_${Date.now()}_${Math.random().toString(36).slice(2)}.sql`;
  try {
    fs.writeFileSync(tmpFile, sql);
    const cmd = `sqlite3 -readonly -json "${OPENCODE_DB_PATH}" < "${tmpFile}"`;
...
function getOpenCodeSessions() {
  ...
  const sessions = rows.map(row => {
    ...
    return {
      ...
      total: (row.input || 0) + (row.output || 0) + (row.cache_read || 0) + (row.cache_write || 0),
      cost: row.cost || 0,
...
function getOpenCodeHistoricalEvents(limit = 10000) {
  ...
  return rows.map(row => {
    ...
    return {
      ...
      cache_write: row.cache_write || 0,
      reasoning: row.reasoning || 0,
      total: row.total || ((row.input||0)+(row.output||0)+(row.cache_read||0)),
