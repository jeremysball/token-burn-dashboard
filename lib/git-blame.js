...
// cache.js requires this module (for GIT_BLAME_CACHE_TTL/primeGitBlameRouteCache),
// so a top-level require('./cache') here would be circular. Resolve lazily
// inside a default-parameter expression instead — by the time any of these
// functions are actually called, both modules have finished loading.
function defaultGitBlameCwd() {
  return require('./cache').GIT_BLAME_WARM_CWD;
}
 ...
 */
function getAvailableDirectories(baseDir = defaultGitBlameCwd()) {
  const dirs = [];
 ...
 */
function getGitCommits(since = '30 days ago', cwd = defaultGitBlameCwd()) {
  try {
 ...
 * @returns {string[]}
 */
function getCommitFiles(commitHash, cwd = defaultGitBlameCwd()) {
 ...
 */
function getCommitLOC(commitHash, cwd = defaultGitBlameCwd()) {
  if (!isValidCommitHash(commitHash)) {
 ...
 */
function getDetectedProjects(days = 30, cwd = defaultGitBlameCwd()) {
...
function computeGitBlameRouteData(days = 30, cwd = defaultGitBlameCwd()) {
...
function getGitBlameRouteData(days = 30, cwd = defaultGitBlameCwd()) {
...
function getGitBlameCommitDetails(commitHash, days = 30, cwd = defaultGitBlameCwd()) {
 ...
 * @returns {object|null}
 */
function getCommitSessionDetails(commitHash, cwd = defaultGitBlameCwd(), days = 30) {
 ...
 */
function generateGitBlameReport(days = 30, cwd = defaultGitBlameCwd()) {
  const commits = getGitCommits(`${days} days ago`, cwd);
  const report = [];
 ...
 */
function getCostByFile(days = 30, cwd = defaultGitBlameCwd()) {
  const commits = getGitCommits(`${days} days ago`, cwd);
