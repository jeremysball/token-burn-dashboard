...
const { SESSIONS_PATHS } = require('./session-paths');
const { getCachedTimeWindow } = require('./session-parser');
...
/**
 * Read the actual session time window from message timestamps, via
 * session-parser's cached parse (calculateSessionTokens already reads and
 * parses the same file moments earlier, so this avoids a second pass).
...
function getSessionTimeWindow(filePath) {
  const window = getCachedTimeWindow(filePath);
