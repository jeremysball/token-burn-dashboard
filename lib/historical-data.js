...
const { parsePiUsage } = require('./session-parser');
              ...
              try {
                ...
                if (msgType === 'message') {
                  ...
                  if (usage && timestamp) {
                    ...
                    const piUsage = parsePiUsage(usage);
                    events.push({
                      time: timestamp,
                      model: modelName,
                      input: piUsage.input,
                      output: piUsage.output,
                      cache_read: piUsage.cacheRead,
                      cache_write: piUsage.cacheWrite,
                      reasoning: piUsage.reasoning,
                      total: piUsage.total
                    });
                  }
                }
