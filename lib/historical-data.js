...
function createHistoricalDataExtractor({ findAllSessionFilesImpl = findAllSessionFiles } = {}) {
  return function extractHistoricalData() {
    return new Promise((resolve, reject) => {
      ...
      try {
        /** @type {any[]} */
        const events = [];
        const sessionFiles = findAllSessionFilesImpl();
        ...
        for (const event of events) {
          const normTime = normalizeTimeMs(event.time);
...
module.exports = { createHistoricalDataExtractor, extractHistoricalData, normalizeTimeMs };
