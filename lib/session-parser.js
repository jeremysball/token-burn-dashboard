...
function getCachedFileData(filePath, options = {}, forceRefresh = false) {
  const now = Date.now();
  if (!forceRefresh && fileDataCache.has(filePath)) {
    const entry = fileDataCache.get(filePath);
    if (now - entry.cachedAt < FILE_DATA_CACHE_TTL) {
      return entry.data;
    }
  }

  const data = parseJsonlFile(filePath, 'auto', options);
  fileDataCache.set(filePath, { data, cachedAt: now });
  return data;
}
