...
function parsePiUsage(usage) {
  ...
  const total = usage.totalTokens != null ? normalizeTokenCount(usage.totalTokens)
    : usage.total != null ? normalizeTokenCount(usage.total)
    : input + output + cacheRead + cacheWrite + reasoning;
...
function parseJsonlFile(filePath, sourceHint = 'auto') {
  const result = {
    ...
    total_lines: 0,
    messages: 0,
    models: {}, // modelKey -> { input, output, cache_read, cache_write, reasoning, total, messages }
    events: [],  // for historical: { time, model, input, output, cache_read, cache_write, reasoning, total }
    firstTimestamp: null,
    lastTimestamp: null,
  ...
  try {
    ...
    for (const line of lines) {
      ...
      result.models[key].reasoning += parsed.reasoning || 0;
