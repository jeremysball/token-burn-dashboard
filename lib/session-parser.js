...
function parseClaudeUsage(usage) {
  ...
  // total = sum if not explicitly provided
...
function parsePiUsage(usage) {
  ...
  // totalTokens may be present, otherwise sum
  const total = usage.totalTokens || usage.total || (input + output + cacheRead + cacheWrite);
