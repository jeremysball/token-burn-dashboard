...
function normalizeModelInfo(data, source) {
  ...
  if (source === 'claude') {
    ...
    if (model.includes('/')) {
      ...
      // The Claude Code session log doesn't record a provider field for a
      // slashless model name, so it can only be inferred from known name
      // patterns — anything else (e.g. a model routed through a proxy,
      // like 'MiniMax-M3') must stay 'unknown' rather than defaulting to
      // 'anthropic' just because the log source is Claude Code.
